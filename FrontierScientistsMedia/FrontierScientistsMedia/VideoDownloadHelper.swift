...
class videoDownloadHelper: NSURLSession, NSURLSessionDownloadDelegate
{
    ...
    func executeBackgroundDownloadForURL()
    {
        ...
        NSUserDefaults.standardUserDefaults().setValue("\(downloadTask.taskIdentifier)", forKey: self.videoTitleString);
    ...
    {
        println("Got into didFinishDownloadingToURL");
        println("downloadTask.taskIdentifier: \(downloadTask.taskIdentifier)");
        
        let libFilePath: String = "Library/Caches/\(self.videoQualityFolder)/\(lastComponentOfUrlString(self.videoUrlString))";
        let VIDFILEPATHSTRING: String = NSHomeDirectory().stringByAppendingPathComponent(libFilePath);
        ...
        dispatch_async(dispatch_get_main_queue(), {
            ...
            NSUserDefaults.standardUserDefaults().setValue("none", forKey: self.videoTitleString);
            })
    }
    ...
    {
        if(session.configuration.identifier == notification.name)
        {
            self.session.invalidateAndCancel();
            dispatch_async(dispatch_get_main_queue(),
                {
